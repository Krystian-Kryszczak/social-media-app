...
class LookFinalizerScreen extends StatefulWidget {
  const LookFinalizerScreen({
    super.key
  });

  @override
  State<LookFinalizerScreen> createState() => _LookFinalizerScreenState();
}

class _LookFinalizerScreenState extends State<LookFinalizerScreen> {
  late TextEditingController textEditingController;

  @override
  void initState() {
    textEditingController = TextEditingController(text: dotenv.maybeGet('TESTING_LOOK_FORM_VALUE'));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: PublishAppBar(onPressed: () => publish(context, textEditingController.text, 'Hello world!',  false, args.first, null), title: Text(Language.getLangPhrase(Phrase.newLook)), iconColor: Theme.of(context).colorScheme.primary, finalize: true),
      body: Column(
  ...
  void publish(BuildContext context, String title, String description, bool private, File content, File? miniature) async {
    Services.lookService.add(title, description, content, miniature, private).onError((error, stackTrace) {
      log(error.toString());
      ...
      AppNavigator.of(context).popAllAndPushNamed(AppRouter.homeRoute)
        .then((_) => messenger.showSnackBar(
          SnackBar(
            content: Text(id != null ? '${Language.getLangPhrase(Phrase.lookSuccessfullyUploaded)} (id: $id)' : Language.getLangPhrase(Phrase.lookUploadFailed)),
